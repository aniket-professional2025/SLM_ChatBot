...
adapter_path = r"C:\Users\Webbies\Jupyter_Notebooks\SLMs\Berger\BergerFineTunnedModel\checkpoint-20"
...
finemodel = finemodel.merge_and_unload()
print("------------------------ Fine-tuned model merged successfully ---------------------------")

# Load the model in Evaluation Mode
finemodel.eval()
print("Runing the Model into Evaluation Model")

# Optimize the model for inference
finemodel = torch.compile(finemodel)
print("Compiling the Model with torch")
...
def get_answer(user_prompt):
    ...
    with torch.no_grad():
        response = finemodel.generate(**tokens, max_new_tokens = 1024, do_sample = True, temperature = 0.7, top_p = 0.9, eos_token_id = tokenizer.eos_token_id)
...
# The New cleaning step added
        # if len(final_output) > 1 and final_output[0].isalpha() and final_output[0].islower() and final_output[1] == ' ':
